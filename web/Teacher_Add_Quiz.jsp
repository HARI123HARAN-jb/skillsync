<%-- 
    Document   : Teacher_Add_Quiz
    Created on : 22-Sep-2025, 15:58:34
    Author     : user
--%>
...
%>
    <p style="color:red;">Please select a course first. <a href="select_course_quiz.jsp">Go Back</a></p>
<%
    } else {
        DbConnection db1 = new DbConnection();
        ResultSet rsCourse = db1.Select("SELECT * FROM courses WHERE course_id='" + courseId + "'");
        String courseName = "";
        if(rsCourse.next()){
...
%>
<br>
<h1>Add Quiz Questions for Course: <%= courseName %></h1>
<form action="Teacher_Add_Quiz" method="post">
    <input type="hidden" name="course_id" value="<%= courseId %>">
    <input type="hidden" name="teacher_id" value="<%= session.getAttribute("teacher_id") %>">
<input type="hidden" name="quiz_type" value="<%= request.getParameter("quiz_type") %>">

    <% for(int i=1; i<=5; i++){ %>
        <h4>Question <%= i %></h4>
        <label>Question:</label><br>
        <textarea name="question<%=i%>" required></textarea><br>
        <label>Option A:</label><input type="text" name="option<%=i%>_a" required><br>
        <label>Option B:</label><input type="text" name="option<%=i%>_b" required><br>
        <label>Option C:</label><input type="text" name="option<%=i%>_c" required><br>
        <label>Option D:</label><input type="text" name="option<%=i%>_d" required><br>
        <label>Correct Answer (A/B/C/D):</label>
        <input type="text" name="answer<%=i%>" maxlength="1" required><br><br>
    <% } %>
